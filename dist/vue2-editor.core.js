...
(function (global, factory) {
  typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('quill'), require('quill-image-resize'), require('quill-image-drop-and-paste'), require('blob-util')) :
  typeof define === 'function' && define.amd ? define(['exports', 'quill', 'quill-image-resize', 'quill-image-drop-and-paste', 'blob-util'], factory) :
  (global = global || self, factory(global.Vue2Editor = {}, global.Quill, global.quillImageResize, global.QuillImageDropAndPaste, global.blobUtil));
}(this, function (exports, Quill, quillImageResize, QuillImageDropAndPaste, blobUtil) { 'use strict';

  Quill = Quill && Quill.hasOwnProperty('default') ? Quill['default'] : Quill;
  QuillImageDropAndPaste = QuillImageDropAndPaste && QuillImageDropAndPaste.hasOwnProperty('default') ? QuillImageDropAndPaste['default'] : QuillImageDropAndPaste;
  ...
  window.hljs.configure({
    ...
    languages: ["javascript", "ruby", "python", "cs", "css", "bash", "shell", "htmlbars"]
  });
  ...
  var script = {
    ...
    methods: {
      ...
      imageHandler: function imageHandler(imageDataUrl, type) {
        // give a default mime type if the type was null
        if (!type) type = "image/png"; // base64 to blob

        var blob = blobUtil.base64StringToBlob(imageDataUrl.replace(/^data:image\/\w+;base64,/, ""), type);
        var index = (this.quill.getSelection() || {}).index || this.quill.getLength();

        if (index) {
          var delta = this.quill.insertEmbed(index, "image", {
            url: imageDataUrl
          }, "user");
          console.log(delta);
        } // var filename = [
